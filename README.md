...
(без конвертации шрифтов - если они есть, то переносятся в результат)
...
(* Далее в путях под `src` подразумевается `allprojects/(ваше название)/src`, а под `dist` - `allprojects/(ваше название)/dist`)
